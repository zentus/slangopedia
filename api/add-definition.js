...
const { encode, createFormData, createDom } = require('../utils')
...
const addDefinition = async data => {
	...
	if (!response.request.path.startsWith('/godkann/?kod=')) {
		return {
			didPost: false,
	...
	const [_, code, definitionId] = response.request.path.match(/\godkann\/\?kod=([0-9].*)x([0-9].*)/)
	const acceptResponse = await axios(`http://www.slangopedia.se/godkann/?kod=${code}x${definitionId}&godkann=ja`)
	const didPost = Boolean(acceptResponse.data.includes('ndringar (Granskas av redakt'))
